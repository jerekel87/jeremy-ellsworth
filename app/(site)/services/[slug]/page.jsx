...
export async function generateMetadata({ params }) {
  ...
  if (!s) return {};
  return {
    title: `${s.title} — Services | je.design`,
    description: s.desc,
  };
}

// scrolling gallery pools per service
const galleryMap = {
  "brand-identity": ["boss-hawgs", "high-caliber", "spartan-hvac", "graybeard", "bison-roofing", "big-spring", "macdavy", "jump-party"],
  "vehicle-wraps": ["salty-soft-wash"],
  "websites": ["zero-gravity", "bags-website"],
  "print-collateral": ["macdavy", "jump-party"],
  "packaging-labels": ["stoopid-energy", "boss-hawgs"],
  "company-apparel": ["graybeard", "jump-party", "bison-roofing", "boss-hawgs"],
};
const wrapExtras = ["/assets/img/revenue/rev-1.jpg", "/assets/img/revenue/rev-5.jpg", "/assets/img/revenue/rev-3.jpg", "/assets/img/revenue/rev-4.jpg", "/assets/img/revenue/rev-6.jpg"];
...
export default async function ServicePage({ params }) {
  ...
  let galleryImgs = (galleryMap[s.slug] || []).map((n) => `/assets/img/work/${n}.jpg`);
  if (s.slug === "vehicle-wraps") galleryImgs = galleryImgs.concat(wrapExtras);
  const phCount = Math.max(0, 8 - galleryImgs.length);
    ...
    <main id="top">
          ...
          <div className="service__gallery">
            <div className="service__galleryTrack">
              <div className="service__galleryGroup">
                {galleryImgs.map((g, gi) => <img key={gi} src={g} alt="" loading="lazy" />)}
      ...
      <section className="casepager">
        <div className="container casepager__row">
          <a href={`/services/${prev.slug}`} className="casepager__link" data-cursor="hover">
          ...
          <a href="/services" className="casepager__all" data-cursor="hover">All services</a>
          <a href={`/services/${next.slug}`} className="casepager__link casepager__link--next" data-cursor="hover">
