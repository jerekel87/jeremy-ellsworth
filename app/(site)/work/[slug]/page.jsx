...
export async function generateMetadata({ params }) {
  ...
  if (!p) return {};
  return { title: `${p.title} — Work | je.design`, description: p.blurb };
}

// after the lead row (image + info panel), stack: 3-up, full, 2-up — repeating
function chunkGallery(images) {
  const pattern = [3, 1, 2];
  const rows = [];
  let i = 0, pi = 0;
...
export default async function ProjectPage({ params }) {
  ...
  const rows = chunkGallery(gallery.slice(1));
    ...
    <main id="top">
        ...
        <div className="container case-gallery">
            ...
            <aside className="cg-panel reveal">
              ...
              <ul className="spec spec--links">
                {(p.services || []).map((slug, si) => {
                  ...
                  return (
                    <li key={slug}>
                      <a href={`/services/${slug}`} data-cursor="hover">
          ...
          {(() => {
            ...
            return rows.map((row, ri) => {
              if (row.length === 1 && !quoteUsed && p.testimonial) {
                quoteUsed = true;
                return (
      ...
      <section className="casepager">
        <div className="container casepager__row">
          <a href={`/work/${prev.slug}`} className="casepager__link" data-cursor="hover">
          ...
          <a href="/work" className="casepager__all" data-cursor="hover">All work</a>
          <a href={`/work/${next.slug}`} className="casepager__link casepager__link--next" data-cursor="hover">
