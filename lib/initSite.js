...
export default function initSite() {
  ...
  if (preloader && !reduceMotion) {
    ...
    var counter = setInterval(function () {
      n.v = Math.min(100, n.v + Math.ceil(Math.random() * 14));
      if (preCount) preCount.textContent = n.v + "%";
      if (n.v >= 100) {
        clearInterval(counter);
        if (pageReady || document.readyState === "complete") finishPreloader();
        else window.addEventListener("load", finishPreloader);
      }
    }, 60);
    window.addEventListener("load", function () { pageReady = true; });
    setTimeout(finishPreloader, 3500); // hard cap
  } else if (preloader) {
    preloader.classList.add("is-done");
    heroIntro();
  }
  ...
  if (burger && mobileMenu) burger.addEventListener("click", function () {
  ...
  if (finePointer && hasGsap && !reduceMotion) {
    ...
    document.querySelectorAll('[data-cursor="hover"], .faq__item summary, .bap-acc__item summary').forEach(function (el) {
  ...
  document.querySelectorAll(".video-card").forEach(function (card) {
    card.addEventListener("click", function () {
      ...
      openVideo(id, h3 ? h3.textContent : "");
    });
  });
  ...
  /* ---------- Service gallery polaroid pop ---------- */
  if (finePointer && hasGsap && !reduceMotion) {
    document.querySelectorAll(".service__galleryGroup").forEach(function (groupEl) {
      var items = Array.prototype.slice.call(groupEl.children);
      items.forEach(function (c, i) {
        if (c.tagName !== "IMG") return;
        c.addEventListener("mouseenter", function () {
          gsap.to(c, {
            scale: 1.12, rotate: i % 2 ? 3 : -3, y: -12, zIndex: 30,
            duration: 0.5, ease: "back.out(2.2)", overwrite: "auto"
          });
          var prev = items[i - 1], next = items[i + 1];
          if (prev) gsap.to(prev, { rotate: -3, x: -10, scale: 0.96, duration: 0.45, ease: "power3.out", overwrite: "auto" });
          if (next) gsap.to(next, { rotate: 3, x: 10, scale: 0.96, duration: 0.45, ease: "power3.out", overwrite: "auto" });
        });
        c.addEventListener("mouseleave", function () {
          [c, items[i - 1], items[i + 1]].forEach(function (el) {
            if (el) gsap.to(el, { scale: 1, rotate: 0, x: 0, y: 0, zIndex: 0, duration: 0.55, ease: "power3.out", overwrite: "auto" });
          });
        });
      });
    });
  ...
  if (workFilter) {
    var workCards = Array.prototype.slice.call(document.querySelectorAll("#workGrid .wcard"));
    workFilter.querySelectorAll(".filter").forEach(function (btn) {
      btn.addEventListener("click", function () {
        workFilter.querySelectorAll(".filter").forEach(function (b) { b.classList.remove("is-active"); });
        ...
        if (hasGsap && !reduceMotion) {
          gsap.fromTo(shown, { y: 24, autoAlpha: 0 }, {
            y: 0, autoAlpha: 1, duration: 0.5, stagger: 0.035, ease: "power3.out", overwrite: "auto", clearProps: "transform"
          });
        }
      });
    });
  ...
  if (drawerEl) {
    ...
    contactForm.addEventListener("submit", function (e) {
      ...
      var body = "Name: " + d.get("name") + "\nEmail: " + d.get("email") +
        ...
        "\nBudget: " + d.get("budget") +
        "\n\n" + (d.get("message") || "");
      location.href = "mailto:inquiry@jeremynellsworth.com?subject=" +
        encodeURIComponent("Project inquiry — " + (d.get("company") || d.get("name"))) +
    ...
    vrow.querySelectorAll(".vslot").forEach(function (el) {
      el.addEventListener("click", function () {
        ...
        if (slot === "vslot--center" || window.matchMedia("(max-width: 820px)").matches) {
          ...
          openVideo(el.getAttribute("data-video"), h3 ? h3.textContent : "");
          return;
        }
