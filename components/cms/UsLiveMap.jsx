...
export default function UsLiveMap({ states }) {
  ...
  const maxState = Math.max(...states.map((s) => s.total));
  const cities = states.flatMap((s) => s.cities);
  const maxCity = Math.max(...cities.map((c) => c.n));
  const cityMode = view.k >= K_CITY_MODE;
  ...
  const { k, x, y } = view;
  /* screen position (in viewBox units) of a world point under the current view */
  ...
  return (
    <div className="cms-map">
      ...
      <div className="cms-map__frame">
        <svg viewBox={US_VIEWBOX} aria-label="Map of live visitors across the United States">
            ...
            {pins.map((p, i) => (
              <g key={p.label} transform={`translate(${p.x}, ${p.y})`}>
                <g className="cms-map__pin" style={{ transform: `scale(${1 / k})` }}>
                  <circle r={p.r + 7} className="cms-map__pulse" style={{ animationDelay: `${i * 0.35}s` }} />
                  <circle r={p.r} className="cms-map__dot" />
                  <text y={0.5} className="cms-map__num" style={{ fontSize: p.r * 1.1 }}>{p.n}</text>
                  <circle
                    r={p.r + 8} className="cms-map__hit"
                    onClick={p.onClick}
        ...
        {tip ? (
          <div
            className="cms-tip cms-map__tip"
      ...
      <span className="cms-map__legend">{cityMode ? "Showing cities · zoom out for state totals" : "One dot per state · click to see cities"}</span>
    </div>
  );
}
