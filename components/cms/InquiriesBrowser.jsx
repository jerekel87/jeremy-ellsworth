...
export default function InquiriesBrowser({ inquiries }) {
    ...
    <div className={"cmsq-app" + (mobileDetail ? " is-detail" : "") + (showInfo ? " has-info" : "")}>
      ...
      <div className="cmsq__list-pane">
        ...
        <ul className="cmsq__list" data-lenis-prevent>
          ...
          {!visible.length && <li className="cmsq__empty">No matching tickets.</li>}
        </ul>
        <button
          type="button"
      ...
      {selected ? (
        <article className="cmsq__detail" key={selected.id} data-lenis-prevent>
          <header className="cmsq__head">
            ...
            <div className="cmsq__head-actions">
              <button type="button" className="cms__btn-ghost cmsq__action">
                  ...
                  out.push(
                    <div className={"cmsq__msg" + (team ? " cmsq__msg--team" : "") + (cont ? " is-cont" : "")} key={i}>
                      ...
                      <div className="cmsq__msg-body">
                        {!cont && (
                          <header>
                            <strong>{team ? "Jeremy Ellsworth" : selected.name}</strong>
                          </header>
                        )}
          ...
          <div className="cmsq__dock">
            <div className="cmsq__composer">
              <div className="cmsq__composer-to">
                ...
                <span className="cmsq__composer-via">via Resend</span>
              </div>
              ...
              <footer className="cmsq__composer-foot">
                <div className="cmsq__composer-left">
                  <button type="button" className="cmsq__iconbtn" title="Attach file">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m21 12-8.5 8.5a6 6 0 0 1-8.5-8.5L12.5 3.5a4 4 0 0 1 5.7 5.7L9.7 17.7a2 2 0 0 1-2.8-2.8L15 7" /></svg>
                  </button>
                  <span className="cmsq__draft-hint">Draft saved</span>
                </div>
                <div className="cmsq__composer-right">
                  <button type="button" className="cms__btn-ghost">Save draft</button>
                  <button type="button" className="btn btn--sm btn--solid"><span>Send reply</span></button>
                </div>
              </footer>
            </div>
          </div>
        ...
        <div className="cmsq__detail cmsq__detail--empty">
          <p>Select a ticket to open it here.</p>
        </div>
      )}
      ...
      {selected && showInfo ? (
        <aside className="cmsq__info" data-lenis-prevent>
          ...
          <div className="cmsq__info-group">
            ...
            <textarea rows={4} placeholder="Notes only your team can see…" defaultValue={selected.note || ""} />
            <button type="button" className="cms__btn-ghost cmsq__info-save">Save note</button>
          </div>
        </aside>
      ) : null}
    </div>
